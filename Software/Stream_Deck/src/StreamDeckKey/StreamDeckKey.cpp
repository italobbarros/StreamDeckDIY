...
void SendBleKey (void){
  if(bleKeyboard.isConnected()){
    ...
    if(AtualizaBLE1 != StatusLCD1){
      if(StatusLCD1){
        Sendkey(KEY_LEFT_CTRL,KEY_F11);
      }
      else{
        Sendkey(KEY_LEFT_CTRL,KEY_F12);
      }
...
void TaskCreateSendBleKey(void){
  xTaskCreatePinnedToCore(vTaskSendBleKey,"vTaskSendBleKey",configMINIMAL_STACK_SIZE+1024+1024+1024,NULL,1,&TaskSendBleKeyHandle,PRO_CPU_NUM);
}
